...
- Known consolidation gap: GitHub issue `#1023` tracks duplicated theme normalization and persistence in `web/templates/layout/base.templ`. The first-paint and interactive paths independently repeat native-theme mapping, legacy `light`/`dark` conversion, custom-ID validation, light/dark inference, `POST /ui/preferences`, and `localStorage` updates. The intended narrow fix is to share normalization, mode derivation, and persistence while preserving synchronous pre-CSS startup and interactive icon/system-theme behavior.
