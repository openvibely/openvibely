...
- `worker_settings.max_workers=0` means unlimited and the API/UI display `Unlimited`. Global, project, and model reservations are atomic and released on completion, failure, cancellation, claim failure, or panic. Project limits are `1..10`; malformed, negative, nonnumeric, or out-of-range inputs fail before side effects. Empty/zero clears a project cap and an increase triggers dispatch; there is no environment override.
