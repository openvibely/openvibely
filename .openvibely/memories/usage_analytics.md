...
- Known Chat inspection gap `#703`: no read-only Chat/runtime action exposes model usage, token, cost, model-breakdown, or account-limit analytics that already exist on `/analytics` and `/api/analytics/usage`.
