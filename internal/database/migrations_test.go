...
func TestMigration100_RepairsSkippedChannelTargetsWhenOldLocalDiscordUsed099(t *testing.T) {
	...
	if maxVersion != 197 {
		t.Fatalf("max goose version = %d, want 197", maxVersion)
	}
}
...
func TestMigration197AddsDurableProviderSteeringLedger(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "provider-steering-197.db")
	db := openMigrationTestDB(t, dbPath)
	...
	if err := goose.UpTo(db, ".", 197); err != nil {
		t.Fatal(err)
	}
	var tableCount int
	if err := db.QueryRow(`SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = 'thread_input_provider_steering'`).Scan(&tableCount); err != nil {
		t.Fatal(err)
	}
	if tableCount != 1 {
		t.Fatalf("provider steering table count = %d, want 1", tableCount)
	}
	if err := goose.DownTo(db, ".", 196); err != nil {
		t.Fatal(err)
	}
	if err := db.QueryRow(`SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = 'thread_input_provider_steering'`).Scan(&tableCount); err != nil {
		t.Fatal(err)
...
func TestMigration108_SystemChannelInboundAuthorizationDedupe(t *testing.T) {
	...
	if err := goose.UpTo(db, ".", 180); err != nil {
...
func TestMigration187CreatesSeparateOAuthConnectionsForExistingModels(t *testing.T) {
	db := openMigrationTestDB(t, filepath.Join(t.TempDir(), "shared-oauth-connections.db"))
	goose.SetBaseFS(migrations.FS)
	defer goose.SetBaseFS(nil)
	if err := goose.SetDialect("sqlite3"); err != nil {
		t.Fatal(err)
	...
	if _, err := db.Exec(`
		...
		VALUES
			('openai-one', 'OpenAI One', 'openai', 'gpt-test', 'oauth', 'access-one', 'refresh-one', 111, 'shared-account', 0, 4),
				('openai-two', 'OpenAI Two', 'openai', 'gpt-test', 'oauth', 'access-two', 'refresh-two', 222, 'shared-account', 1, 7),
				('anthropic-one', 'Anthropic One', 'anthropic', 'claude-test', 'oauth', 'access-three', 'refresh-three', 333, 'shared-org', 0, 9);
			INSERT INTO account_usage_snapshots (id, provider, account_id, agent_config_id, oauth_config_revision, raw_json)
			VALUES ('openai-one-snapshot', 'openai', 'shared-account', 'openai-one', 4, '{}');	`); err != nil {
		t.Fatalf("seed OAuth configs: %v", err)
	}
	if err := goose.UpTo(db, ".", 187); err != nil {
		t.Fatalf("migrate to 187: %v", err)
	}
	rows, err := db.Query(`
		SELECT a.id, a.oauth_connection_id, c.provider, c.oauth_access_token,
		       c.oauth_refresh_token, c.oauth_expires_at, c.oauth_account_id,
		       c.oauth_needs_reauth, c.oauth_revision
		FROM agent_configs a JOIN oauth_connections c ON c.id = a.oauth_connection_id
		WHERE a.id IN ('openai-one', 'openai-two', 'anthropic-one') ORDER BY a.id`)
	if err != nil {
		t.Fatalf("query migrated OAuth connections: %v", err)
	}
	defer rows.Close()
	seen := map[string]string{}
	for rows.Next() {
		var id, connectionID, provider, access, refresh, account string
		var expires, revision int64
		var needsReauth bool
		if err := rows.Scan(&id, &connectionID, &provider, &access, &refresh, &expires, &account, &needsReauth, &revision); err != nil {
			t.Fatalf("scan migrated OAuth connection: %v", err)
		}
		if connectionID != id {
			t.Fatalf("model %s connection = %q, want private connection %q", id, connectionID, id)
		}
		seen[id] = access
	}
	if len(seen) != 3 || seen["openai-one"] != "access-one" || seen["openai-two"] != "access-two" || seen["anthropic-one"] != "access-three" {
		t.Fatalf("migrated connections = %+v", seen)
	}
	var duplicateCredentials int
	if err := db.QueryRow(`
		SELECT COUNT(*) FROM agent_configs
		WHERE id IN ('openai-one', 'openai-two', 'anthropic-one')
		  AND (oauth_access_token != '' OR oauth_refresh_token != '' OR oauth_expires_at != 0 OR oauth_account_id != '' OR oauth_needs_reauth != 0)`).Scan(&duplicateCredentials); err != nil {
		t.Fatalf("query legacy OAuth fields: %v", err)
	}
	if duplicateCredentials != 0 {
		t.Fatalf("%d migrated model rows retained duplicate OAuth state", duplicateCredentials)
	}
	var snapshotConnectionID string
	if err := db.QueryRow(`SELECT oauth_connection_id FROM account_usage_snapshots WHERE id = 'openai-one-snapshot'`).Scan(&snapshotConnectionID); err != nil {
		t.Fatalf("query migrated snapshot connection: %v", err)
	}
