...
func TestBuiltInGitHubAutonomousSDLCBootstrapSkillContent(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Call `set_task_goal` for a newly created task or a reconciled task that is not completed",
		"Never call `set_task_goal` for a reconciled completed task; preserve its existing goal and terminal goal status",
