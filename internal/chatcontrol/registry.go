...
var registry = []ActionDef{
	...
	{
		Name:         "list_alerts",
		Description:  "List project-scoped alerts and actionable notifications with stable pagination and conjunctive lifecycle filters. Provider-visible optional filters use a non-semantic all/empty omission sentinel that is removed before handler dispatch. Native Approved Inbox callers use decision_state=approved and implementation_task_linked=unlinked and leave other filters at their omission sentinel so eligible work is not excluded.",
		Domain:       DomainAlerts,
		Access:       AccessRead,
		Sensitivity:  SensitivityNormal,
		AllowedModes: bothModes(),
		Surfaces:     allSurfaces(),
		Parameters:   json.RawMessage(`{"type":"object","properties":{"project_id":{"type":"string","default":"","x-openvibely-omit-value":"","description":"Optional same-project assertion. Leave empty to use the persisted caller task's project context; never discover or reuse a project ID."},"decision_state":{"type":"string","enum":["all","not_required","pending","approved","rejected","dismissed"],"default":"all","x-openvibely-omit-value":"all","description":"Decision-state filter. Use all for no decision-state filter."},"processing_state":{"type":"string","enum":["all","not_applicable","unclaimed","claimed","implementation_task_linked","completed","failed"],"default":"all","x-openvibely-omit-value":"all","description":"Processing-state filter. Use all for no processing-state filter."},"type":{"type":"string","default":"","x-openvibely-omit-value":"","description":"Optional notification-type filter. Leave empty to include all types."},"source":{"type":"string","default":"","x-openvibely-omit-value":"","description":"Optional notification-source filter. Leave empty to include all sources."},"read":{"type":"string","enum":["all","read","unread"],"default":"all","x-openvibely-omit-value":"all","description":"Read-state filter. Use all to include both read and unread alerts."},"implementation_task_linked":{"type":"string","enum":["all","linked","unlinked"],"default":"all","x-openvibely-omit-value":"all","description":"Implementation-task linkage filter. Use all for no linkage filter."},"limit":{"type":"integer","minimum":1,"maximum":100},"offset":{"type":"integer","minimum":0}},"additionalProperties":false}`),
	},
	{
		Name:         "get_alert",
