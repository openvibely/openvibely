...
var registry = []ActionDef{
	...
	{
		Name:         "list_alerts",
		Description:  "List project-scoped alerts and actionable notifications with stable pagination and conjunctive lifecycle filters. Optional filter values may be null to omit them. Native Approved Inbox callers use decision_state=approved and implementation_task_linked=false and omit or pass null for processing_state, read, type, and source so eligible work is not excluded.",
		Domain:       DomainAlerts,
		Access:       AccessRead,
		Sensitivity:  SensitivityNormal,
		AllowedModes: bothModes(),
		Surfaces:     allSurfaces(),
		Parameters:   json.RawMessage(`{"type":"object","properties":{"project_id":{"type":["null","string"],"default":null,"description":"Optional same-project assertion. Omit it or pass null to use the persisted caller task's project context; never discover or reuse a project ID."},"decision_state":{"type":["null","string"],"enum":[null,"not_required","pending","approved","rejected","dismissed"],"default":null,"description":"Optional decision-state filter. Omit it or pass null to include all decision states."},"processing_state":{"type":["null","string"],"enum":[null,"not_applicable","unclaimed","claimed","implementation_task_linked","completed","failed"],"default":null,"description":"Optional processing-state filter. Omit it or pass null to include all processing states."},"type":{"type":["null","string"],"default":null,"description":"Optional notification-type filter. Omit it or pass null to include all types."},"source":{"type":["null","string"],"default":null,"description":"Optional notification-source filter. Omit it or pass null to include all sources."},"read":{"type":["null","boolean"],"default":null,"description":"Optional read-state filter. Omit it or pass null to include both read and unread alerts."},"implementation_task_linked":{"type":["null","boolean"],"default":null,"description":"Optional implementation-task linkage filter. Omit it or pass null to include both linked and unlinked alerts."},"limit":{"type":"integer","minimum":1,"maximum":100},"offset":{"type":"integer","minimum":0}},"additionalProperties":false}`),
	},
	{
		Name:         "get_alert",
