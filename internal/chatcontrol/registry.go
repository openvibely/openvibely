...
var registry = []ActionDef{
	...
	// --- Alerts and actionable notifications domain ---
	{
		...
		AllowedModes: bothModes(),
		Surfaces:     allSurfaces(),
		Parameters:   json.RawMessage(`{"type":"object","properties":{"project_id":{"type":["string","null"],"description":"Optional same-project assertion. Omit it or pass null to use the persisted caller task's project context; never discover or reuse a project ID."},"decision_state":{"type":["string","null"],"enum":["not_required","pending","approved","rejected","dismissed",null],"description":"Optional decision-state filter. Omit it or pass null to include all decision states."},"processing_state":{"type":["string","null"],"enum":["not_applicable","unclaimed","claimed","implementation_task_linked","completed","failed",null],"description":"Optional processing-state filter. Omit it or pass null to include all processing states."},"type":{"type":["string","null"],"description":"Optional notification-type filter. Omit it or pass null to include all types."},"source":{"type":["string","null"],"description":"Optional notification-source filter. Omit it or pass null to include all sources."},"read":{"type":["boolean","null"],"description":"Optional read-state filter. Omit it or pass null to include both read and unread alerts."},"implementation_task_linked":{"type":["boolean","null"],"description":"Optional implementation-task linkage filter. Omit it or pass null to include both linked and unlinked alerts."},"limit":{"type":"integer","minimum":1,"maximum":100},"offset":{"type":"integer","minimum":0}},"additionalProperties":false}`),
	},
	{
		Name:         "get_alert",
