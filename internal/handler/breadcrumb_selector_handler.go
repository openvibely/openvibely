...
func (h *Handler) GetBreadcrumbSelectorResults(c echo.Context) error {
	...
	case "tasks":
		...
		items, err = h.taskSvc.ListBreadcrumbSelector(c.Request().Context(), projectID, search, currentID, breadcrumbSelectorLimit+1)
	case "automations":
		kind = "Automation"
	...
	for i := range items {
		items[i].URL = breadcrumbSelectorItemURL(c.Param("resource"), items[i].ID, projectID, c.QueryParam("tab"), c.QueryParam("view"))
	}
	return render(c, http.StatusOK, components.BreadcrumbSelectorResults(kind, currentID, items, hasMore))
}

func breadcrumbSelectorItemURL(resource, id, projectID, tab, view string) string {
