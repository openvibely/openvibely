...
func TestRequestUserInputToolWebOnlyRegistryAndHandler(t *testing.T) {
	...
	for _, def := range webDefs {
		...
		require.JSONEq(t, `{"type":"object","required":["questions"],"additionalProperties":false,"properties":{"questions":{"type":"array","description":"Ask 1 to 3 questions. Put the recommended/default answer first for each question. Do not add a generic per-question move-forward option unless it is a real answer to that specific question; the Chat UI provides a separate Recommended and move forward shortcut for submitting all first-option recommendations. These choices are not answers until the user explicitly selects them.","minItems":1,"maxItems":3,"items":{"type":"object","required":["id","question","options"],"additionalProperties":false,"properties":{"id":{"type":"string","description":"Stable question id used in the answer result."},"question":{"type":"string","description":"Plain-text question to show to the user."},"options":{"type":"array","description":"Clickable choices. The first option must be the recommended/default answer. Do not treat any option as selected until the user clicks it or explicitly chooses the global Recommended and move forward shortcut.","minItems":2,"maxItems":3,"items":{"type":"object","required":["label","description"],"additionalProperties":false,"properties":{"label":{"type":"string","description":"Clickable option label returned exactly when selected."},"description":{"type":"string","description":"Short plain-text explanation shown under the label."}}}}}}}}}`, string(def.Parameters))
		description := strings.ToLower(def.Description)
		require.Contains(t, description, "recommended/default")
		require.Contains(t, description, "recommended and move forward shortcut")
		require.Contains(t, description, "do not add a generic per-question move-forward")
		require.Contains(t, description, "model/agent")
		require.Contains(t, description, "persistent goal")
		require.Contains(t, description, "each task-creation question must put")
		require.Contains(t, description, "do not assume, preselect, or answer")
		require.Contains(t, description, "explicitly select each answer")
	}
