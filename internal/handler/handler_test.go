...
func TestHandler_Analytics_FullPage(t *testing.T) {
	...
	assertContains(t, rec, "Provider limits, local token usage, known cost, and observed outcomes.")
	assertContains(t, rec, `id="accountUsageCards" class="grid grid-cols-1 lg:grid-cols-2 gap-6"`)
	...
	assertContains(t, rec, "Known failed-run cost")
