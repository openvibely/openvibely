...
func TestHandler_Analytics_FullPage(t *testing.T) {
	...
	assertContains(t, rec, "Provider limits, token usage, cache utilization, and recorded cost.")
	assertContains(t, rec, `id="accountUsageCards" class="relative grid grid-cols-1 lg:grid-cols-2 gap-6"`)
	assertNotContains(t, rec, `data-analytics-view="all"`)
	assertNotContains(t, rec, ">All Metrics</button>")
	...
	assertNotContains(t, rec, "Known failed-run cost")
	assertContains(t, rec, "Model comparison")
	assertContains(t, rec, "Saved model configuration")
	assertNotContains(t, rec, "Exact model values")
...
func TestSidebar_ProjectSelectorSearchTriggerAndFocusVisible(t *testing.T) {
	...
	required := []string{
		...
		`class="select select-bordered select-sm w-full sidebar-project-select flex min-w-0 items-center text-left"`,
