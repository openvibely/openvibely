...
func TestAutomationPagesRenderRegisteredDefinitionsAndEnforceProject(t *testing.T) {
	...
	mainContentStart := strings.Index(detailBody, `<main id="main-content"`)
	require.GreaterOrEqual(t, mainContentStart, 0, "automation detail must render the main content container")
	mainContent := detailBody[mainContentStart:]
	mainContentEnd := strings.Index(mainContent, `</main>`)
	require.GreaterOrEqual(t, mainContentEnd, 0, "automation detail must close the main content container")
