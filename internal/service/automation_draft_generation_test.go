...
func TestAutomationDescriptionPromptMatchesStrictCustomValidationContract(t *testing.T) {
	...
	_, err = svc.PreviewDescription(context.Background(), "Review a request", models.AutomationCapabilitySnapshot{}, func(_ context.Context, prompt string) (string, error) {
		...
		require.Contains(t, prompt, "Call list_alerts without project_id, using decision_state=approved, processing_state=unclaimed, implementation_task_linked=false")
		require.Contains(t, prompt, "Do not pass read, type, or source")
		require.Contains(t, prompt, "failed notifications require a separate recovery query")
