...
func TestNativeSDLCTemplateUsesAutomationOwnedPromptsMatchingBootstrapContract(t *testing.T) {
	...
	require.Contains(t, inboxPrompt, `Call list_alerts using project_id="", decision_state=approved, processing_state=all`)
	...
	require.Contains(t, inboxPrompt, "provider/runtime boundary removes the non-semantic empty/all values")
