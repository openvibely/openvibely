...
Call list_alerts without project_id, using decision_state=approved, processing_state=unclaimed, implementation_task_linked=false, a bounded limit, and stable pagination. Do not pass read, type, or source: omitting those filters includes both read states. The primary inbox query processes newly approved, unclaimed notifications; failed notifications require a separate recovery query. The runtime automatically uses this scheduled task's persisted project and Automation ownership. Never reuse a project ID from prior messages, examples, memory, or tool output. Before calling claim_alert, collect every eligible result from all pages by following the returned pagination offsets. Do not claim, link, or process any notification while paginating because linkage removes rows from this filtered result set and advancing an offset after mutation can skip notifications. Only after the complete paginated snapshot is collected, call get_alert for each collected notification and inspect the full body and metadata before claiming it.
