...
Call list_alerts with only a bounded limit, offset, and stable pagination. Do not pass project_id, decision_state, processing_state, implementation_task_linked, read, type, or source: the Native Approved Inbox runtime enforces the current project and Automation ownership, approved and unlinked state, both read states, and all recovery-eligible processing states. Never reuse a project ID from prior messages, examples, memory, or tool output. Before calling claim_alert, collect every eligible result from all pages by following the returned pagination offsets. Do not claim, link, or process any notification while paginating because linkage removes rows from this filtered result set and advancing an offset after mutation can skip notifications. Only after the complete paginated snapshot is collected, call get_alert for each collected notification and inspect the full body and metadata before claiming it.
