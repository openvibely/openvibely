...
func TestBuildOpenAIRuntimeAdvertisesAndExecutesMCPToolsOnce(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "initialize":
			...
			writeOpenAIMCPResult(t, w, req.ID, map[string]any{"protocolVersion": "2024-11-05", "capabilities": map[string]any{}})
		case "tools/list":
			toolsListCalls.Add(1)
			writeOpenAIMCPResult(t, w, req.ID, map[string]any{"tools": []map[string]any{{"name": "screenshot", "description": "Capture browser", "inputSchema": map[string]any{"type": "object"}}}})
		case "tools/call":
			toolsCallCalls.Add(1)
			writeOpenAIMCPResult(t, w, req.ID, map[string]any{"content": []map[string]string{{"type": "text", "text": "captured"}}, "isError": false})
		default:
	...
	extraTools, executeTool, allowTool, cleanup := buildOpenAIRuntime(context.Background(), t.TempDir(), agentDef)
	defer cleanup()
	...
	if len(extraTools) != 1 || extraTools[0].Name != "browser__screenshot" || extraTools[0].Type != "function" {
		t.Fatalf("unexpected OpenAI MCP tool definitions: %#v", extraTools)
	}
...
func writeOpenAIMCPResult(t *testing.T, w http.ResponseWriter, id int64, result any) {
	t.Helper()
