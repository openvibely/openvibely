...
type testAstraAsyncTracker struct{}

func (testAstraAsyncTracker) AstraAsyncToolCallTrackingEnabled(name string) bool {
	return strings.EqualFold(strings.TrimSpace(name), "read_task") || strings.EqualFold(strings.TrimSpace(name), "write_task")
}

func TestRuntimeOpenAIToolsWithAstraAsyncRequiresReadRuntimeToolAndTracker(t *testing.T) {
	rt := &llmcontracts.RuntimeTools{Definitions: []llmcontracts.RuntimeToolDefinition{
		{Name: "read_task", Access: llmcontracts.RuntimeToolAccessRead},
		{Name: "write_task", Access: llmcontracts.RuntimeToolAccessWrite},
		{Name: "implicit_write"},
	}}
	withoutTracker := runtimeOpenAIToolsWithAstraAsync(rt, nil)
	for _, tool := range withoutTracker {
		if tool.Async {
			t.Fatalf("tool %s was async without durable tracker", tool.Name)
		}
	}
	withTracker := runtimeOpenAIToolsWithAstraAsync(rt, testAstraAsyncTracker{})
	asyncByName := map[string]bool{}
	for _, tool := range withTracker {
		asyncByName[tool.Name] = tool.Async
	}
	if !asyncByName["read_task"] {
		t.Fatalf("read_task async = false, want true with tracker")
	}
	if asyncByName["write_task"] || asyncByName["implicit_write"] {
		t.Fatalf("write tools must not become async: %#v", asyncByName)
	}
	if tracker := astraAsyncRuntimeTracker(&llmcontracts.RuntimeTools{Metadata: testAstraAsyncTracker{}}, models.LLMConfig{Model: "gpt-5.6-sol"}); tracker != nil {
		t.Fatal("non-Astra model unexpectedly enabled async tracker")
	}
	if tracker := astraAsyncRuntimeTracker(&llmcontracts.RuntimeTools{Metadata: testAstraAsyncTracker{}}, models.LLMConfig{Model: "gpt-6-astra"}); tracker == nil {
		t.Fatal("Astra model with tracker metadata should enable async tracker")
	}
}
