...
- For task-creation clarification, unless the user already specified the answer, the request_user_input call MUST include questions for: which model/agent should run it, whether to assign a persistent goal, and whether/how to create the task with sensible defaults for category/priority/scheduling/chaining/auto-merge where relevant. Each of those questions MUST put the recommended/default option first and include a move-forward option that proceeds with sensible defaults.
