...
func TestApplyChatActionToolModeRequiresStructuredClarificationForProposedWork(t *testing.T) {
	...
	for _, want := range []string{
		...
		`Do not add a generic per-question "move forward" option`,
		`separate "Recommended and move forward" control`,
		"do not assume, preselect, or answer any option on the user's behalf",
		"inspect every answer before responding",
		"leaves a requested decision unresolved",
		"Do not give a final summary or tell the user to ask again",
		"which model/agent should run it", "whether to assign a persistent goal",
		"whether/how to create the task with sensible defaults",
		"Each of those questions MUST put the recommended/default option first",
		"choose the global recommended shortcut",
		"MUST call request_user_input instead of writing the questions as ordinary assistant prose", "This rule still applies after memory, file, search, or other read-only tool calls",
