...
func (i *LLMHookInvoker) resolveLLMConfig(ctx context.Context, hook models.AgentLifecycleHook) (models.LLMConfig, *models.Agent, error) {
	...
	if i.agents != nil && hook.AgentID != "" {
		if a, err := i.agents.GetByID(ctx, hook.AgentID); err == nil && a != nil {
			...
			if model != "" && !strings.EqualFold(model, "inherit") {
				...
				if isPersistedLLMConfigID(model) {
					return i.resolveDefaultForMissingModel(ctx, agentDef, model)
				}
	...
	return i.resolveDefaultForMissingModel(ctx, agentDef, "")
}

func (i *LLMHookInvoker) resolveDefaultForMissingModel(ctx context.Context, agentDef *models.Agent, missingModel string) (models.LLMConfig, *models.Agent, error) {
	if i.models != nil {
		if def, err := i.models.GetDefault(ctx); err != nil {
			return models.LLMConfig{}, agentDef, err
		} else if def != nil {
	...
	if missingModel != "" {
		return models.LLMConfig{}, agentDef, fmt.Errorf("lifecycle: configured model %q for Agent %q is unavailable and no default model is configured", missingModel, agentDef.Name)
	}
	return models.LLMConfig{Model: "inherit"}, agentDef, nil
}

// LLM config IDs are generated by the repository as lower-case hex(randomblob(16)).
// Values outside that representation are retained for compatibility with legacy
// Agent rows that stored provider model slugs directly.
func isPersistedLLMConfigID(value string) bool {
	if len(value) != 32 {
		return false
	}
	for _, c := range value {
