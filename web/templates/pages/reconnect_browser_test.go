...
window.addEventListener('DOMContentLoaded', async function() {
  ...
  try {
    ...
    if (root.getAttribute('data-chat-revision') !== expectedRevision) {
      ...
      var currentError = pair.querySelector('[data-terminal-error="true"]');
      var nextError = nextPair ? nextPair.querySelector('[data-terminal-error="true"]') : null;
      fail('Chat terminal revision stayed stale: current=' + root.getAttribute('data-chat-revision') + ' expected=' + expectedRevision + ' match=' + window.chatTranscriptSnapshotMatches(root, nextRoot, 'chat-messages', '` + execID + `') + ' currentRaw=' + (currentOutput && currentOutput.getAttribute('data-raw-content')) + ' nextRaw=' + (nextOutput && nextOutput.getAttribute('data-raw-content')) + ' currentError=' + (currentError && currentError.textContent) + ' nextError=' + (nextError && nextError.textContent) + ' currentExecRevision=' + pair.getAttribute('data-execution-revision') + ' nextExecRevision=' + (nextPair && nextPair.getAttribute('data-execution-revision')) + ' currentStatus=' + pair.getAttribute('data-exec-status') + ' nextStatus=' + (nextPair && nextPair.getAttribute('data-exec-status')) + ' syncResults=' + JSON.stringify(window.__syncResults) + ' known=' + !!window._chatKnownExecIds['` + execID + `'] + ' ajax=' + JSON.stringify(window.__ajaxCalls));
    }
    var swapsBeforeRefocus = window.__swaps.length;
    window.__hiddenToVisible();
    await window.__wait(100);
