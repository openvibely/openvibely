...
window.addEventListener('load',async function(){
 ...
 try{
  ...
  var tokenTrend=configs.usageRateChart;
  assert(tokenTrend.plugins.includes(hoverPlugin),'similar line charts should share the same hover plugin');
  for(var option of ['tension','cubicInterpolationMode','borderWidth','pointRadius','pointHoverRadius','fill'])assert(trend.data.datasets[0][option]===tokenTrend.data.datasets[0][option],'line style drift: '+option);
  assert(trend.options.plugins.tooltip.position===tokenTrend.options.plugins.tooltip.position&&trend.options.interaction.mode===tokenTrend.options.interaction.mode,'line interaction defaults should match');
...
func TestAnalyticsContent_LineChartHoverMarkerPaintsAfterTooltip(t *testing.T) {
	...
	if got := strings.Count(content, `new Chart(`); got != 1 {
		t.Fatalf("all charts must go through the shared chart factory, got %d constructors", got)
	}
	if !strings.Contains(content, `config.plugins=[...(config.plugins||[]),analyticsActivePointOnTop]`) {
		t.Fatal("shared line chart defaults must install hover behavior")
	}
