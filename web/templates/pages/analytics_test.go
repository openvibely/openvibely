...
window.addEventListener('load',async function(){
 ...
 try{
  ...
  assert(usageRows[0].cells.length===7&&usageRows[0].cells[0].querySelector('strong').textContent==='model'&&usageRows[0].cells[0].querySelector('.text-xs').textContent==='openai','model and provider should share one identity column');
  ...
  document.dispatchEvent(new KeyboardEvent('keydown',{key:'Escape',bubbles:true}));
  assert(usageRows[0].cells[1].textContent.includes('1M')&&usageRows[0].cells[1].querySelector('[data-model-help]').dataset.modelHelp.includes('1,000,000'),'compact values must retain exact counts in help');
  assert(usageRows[0].cells[1].querySelector('[data-model-help]').dataset.modelHelp==='1,000,000','cell help must contain only the exact value');
  assert(usageRows[0].cells[3].querySelector('[data-model-help]').dataset.modelHelp==='900,000','cache explanation belongs only in the column help');
