  ...
  window.Chart=function(ctx,config){this.destroy=function(){};if(ctx&&ctx.canvas)charts[ctx.canvas.id]=config;};
    ...
    return Promise.resolve({ok:true,json:function(){return Promise.resolve(payload);}});
	  ...
	  window.addEventListener('load',function(){waitFor(function(){return charts.hourlyTrendsChart&&charts.skillUsageTrendChart&&charts.skillTopChart&&charts.skillFollowChart&&charts.skillAgentChart&&charts.usageRateChart&&charts.modelTokenBreakdownChart;},function(){
	    var outcomeLink=document.querySelector('#skillOutcomeTable a');var pairLink=document.querySelector('#agentSkillOutcomeTable a[data-skill-evidence-link]');var agentLink=document.querySelector('#agentSkillTable a[data-skill-evidence-link]');
	    ...
	    if(outcomeParams.get('skill_handle')!==longHandle||outcomeParams.get('skill_evidence_scope')!=='global'||outcomeParams.get('skill_event')!=='used')fail('skill outcome link lost scoped evidence identity');
	    if(pairParams.get('skill_handle')!==longHandle||pairParams.get('skill_evidence_scope')!=='project'||pairParams.get('skill_agent')!=='agent-1'||pairParams.get('skill_event')!=='used')fail('Agent-skill outcome link lost scoped evidence identity');
	    if(agentParams.get('skill_handle')!==longHandle||agentParams.get('skill_evidence_scope')!=='agent_owned'||agentParams.get('skill_agent')!=='agent-1'||agentParams.get('skill_event')!=='used')fail('selected Agent skill link lost scoped evidence identity');
