...
func TestAnalyticsContent_ChartInteractionsPreserveExactSupportingSubsetInChrome(t *testing.T) {
	...
	fixture := `<main id="reconnect-result"></main><script>
(function(){
	  var result=document.getElementById('reconnect-result'),charts={},urls=[];  function fail(message){result.setAttribute('data-test-result','fail');result.setAttribute('data-test-error',message);throw new Error(message);}
  window.Chart=function(ctx,config){this.destroy=function(){};if(ctx&&ctx.canvas)charts[ctx.canvas.id]=config;};
	  window.fetch=function(url){var value=String(url),query=new URL(value,location.href).searchParams,payload=[];urls.push(value);
	    ...
	    if(value.indexOf('/api/analytics/skills')>=0)payload={usage_over_time:[{period:'2026-01-10',selected_count:1,loaded_count:0,viewed_count:0,created_count:2,edited_count:0}],top_skills:[{skill_handle:'shared:review',skill_scope:'project',selected_count:1,loaded_count:1,viewed_count:0,created_count:2,edited_count:0},{skill_handle:'shared:review',skill_scope:'global',selected_count:0,loaded_count:0,viewed_count:0,created_count:1,edited_count:0}],follow_through:[{skill_handle:'shared:review',skill_scope:'project',selected_count:2,loaded_or_viewed_count:1,ignored_count:1},{skill_handle:'shared:review',skill_scope:'global',selected_count:1,loaded_or_viewed_count:0,ignored_count:1}],agent_usage:{agents:[{agent_id:'agent-1',agent_name:'Agent One'}],cells:[{agent_id:'agent-1',skill_handle:'project:review',selected_count:1,loaded_count:0,viewed_count:0}]},underused:[],evidence_total:query.get('skill_period')||query.get('skill_handle')?1:0,evidence:query.get('skill_period')||query.get('skill_handle')?[{id:'skill-event-1',created_at:'2026-01-10T10:00:00Z',task_id:'skill-task',execution_id:'skill-exec',agent_id:'agent-1',agent_name:'Agent One',skill_handle:query.get('skill_handle')||'project:created',skill_scope:query.get('skill_evidence_scope')||'project',event_type:query.get('skill_event')||'selected',source:'manual',surface:'task_thread'}]:[]};
    ...
    if(params.get('evidence')!=='execution_hour'||params.get('execution_hour')!=='10'||document.getElementById('outcomeEvidenceTable').textContent.indexOf('Hour task')<0)fail('hour click did not filter exact task evidence');
	    ...
	    charts.skillAgentChart.options.onClick({},[{index:0,datasetIndex:0}]);
		    waitFor(function(){return urls.some(function(url){return url.indexOf('/api/analytics/skills')>=0&&url.indexOf('skill_agent=agent-1')>=0&&url.indexOf('skill_handle=project%3Areview')>=0&&url.indexOf('skill_event=used')>=0;})&&document.getElementById('skillEventEvidenceTable').textContent.indexOf('project:review')>=0;},function(){	    params=new URLSearchParams(location.search);
	    if(params.get('skill_agent')!=='agent-1'||params.get('skill_handle')!=='project:review'||document.getElementById('skillEvidenceSelection').textContent.indexOf('Agent One')<0)fail('Agent-skill click lost pair identity or supporting records');
	    charts.usageRateChart.options.onClick({},[{index:0,datasetIndex:0}]);
	    ...
	    if(params.get('usage_period')!=='2026-01-10'||params.get('usage_model')!=='combined'||document.getElementById('usageEvidenceSelection').textContent.indexOf('2026-01-10')<0)fail('token trend click lost period/model aggregate or supporting records');
		    ...
		    waitFor(function(){return urls.some(function(url){return url.indexOf('/api/analytics/skills')>=0&&url.indexOf('skill_handle=shared%3Areview')>=0&&url.indexOf('skill_evidence_scope=project')>=0&&url.indexOf('skill_event=created')>=0;})&&document.getElementById('skillEventEvidenceTable').textContent.indexOf('project')>=0;},function(){
		      var skillParams=new URLSearchParams(location.search);
		      if(skillParams.get('skill_evidence_scope')!=='project'||charts.skillTopChart.data.labels[0]===charts.skillTopChart.data.labels[1])fail('Top Skills click lost scope identity or rendered ambiguous duplicate labels');
		      charts.skillFollowChart.options.onClick({},[{index:1,datasetIndex:1}]);
		      waitFor(function(){return urls.some(function(url){return url.indexOf('/api/analytics/skills')>=0&&url.indexOf('skill_handle=shared%3Areview')>=0&&url.indexOf('skill_evidence_scope=global')>=0&&url.indexOf('skill_event=ignored')>=0;})&&document.getElementById('skillEventEvidenceTable').textContent.indexOf('global')>=0;},function(){	        charts.modelTokenBreakdownChart.options.onClick({},[{index:0}]);
