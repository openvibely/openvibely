...
- **Provider-native web search**: Both OpenAI and Anthropic adapters include provider-executed web tools when `WebSearchEnabled: true` (default). OpenAI now sends `{"type":"web_search"}` (legacy `web_search_preview` kept only as compatibility alias in mappings), and round-trips provider output items (`web_search_call`/`tool_search_call`) without local execution. OpenAI web-search tool callbacks emit summary detail (`query`/`url`) from `response.output_item.added` when present, but defer tool-use emission to `.done` when `.added` is sparse so the UI still gets secondary detail. OpenAI web-search tool-result text is status-focused (`status`, optional `sources`) to avoid raw `url="..."`/`query="..."` blobs in OUT cards. Anthropic now sends direct versioned tool types (currently `web_search_20250305` + `web_fetch_20250910` with names `web_search`/`web_fetch`) via mixed `rawTools` JSON marshaling; do not wrap these as `{"type":"server_tool"}`. `server_tool_use` and newer provider tool-result blocks are parsed/round-tripped without local execution. Web search tools are allowed in plan mode (read-only). Centralized constants in `internal/llm/websearch/websearch.go`.
- **Anthropic provider tool-result round-trip**: provider-managed result block types are matched generically as `*_tool_result` (for example `code_execution_tool_result`, `web_fetch_tool_result`) and carried forward in assistant history between `pause_turn` continuations so provider-side loops remain protocol-valid.
- Anthropic provider tool-result blocks must preserve the original JSON shape of `content` when round-tripped (for example `code_execution_tool_result.content` objects). Replaying these payloads as plain strings causes `RequestCodeExecutionToolResultError` invalid-shape failures on subsequent turns.
- Anthropic tool callbacks for provider-native web/code tools now fire even when the turn ends with `stop_reason="end_turn"` (not just `tool_use`/`pause_turn`), so UI tool cards still appear when provider execution completed within the final response turn.
- Anthropic provider tool callbacks must be emitted in stream order (at `content_block_stop` while parsing), not in a post-turn pass; delayed callback emission can render tool cards after summary text in chat/thread UI.
- Anthropic agentic loop no longer injects forced provider-note retries for URL prompts after `code_execution`/`bash_code_execution` `too_many_requests`; tool routing remains model-driven.
- Anthropic adapters no longer append explicit “Provider web retrieval guidance” prose to system prompts; web-tool behavior is enforced in runtime/tool-loop logic to avoid user-visible leakage of implementation guidance.
- Anthropic task execution and task-thread follow-ups no longer suppress default local coding tools based on URL presence. `SkipDefaultTools` is mode-driven for orchestrate chat with runtime action tools.
- Anthropic OAuth requests now include `x-app: cli` header (in both streaming agentic and non-agentic send paths) to stay aligned with Claude Code request shape.
- Anthropic orchestrate chat with runtime action tools now sends runtime tools without default local coding tools (`read_file`/`write_file`/`edit_file`/`bash`/`list_files`/`grep_search`) to avoid wasted "tool not allowed" turns; task follow-up and plan modes keep existing tool behavior.
- **Anthropic server-tool continuation**: when Anthropic returns `stop_reason="pause_turn"` during provider-side tool loops (for example long web-search/fetch chains), the client must continue the agentic loop with the same conversation state (no extra user message) so server-side execution resumes instead of returning partial "execution limit for this turn" style text.
...
- **Worktree prompt context (API paths)**: OpenAI/Anthropic task system prompts now append explicit orientation text when a worktree `workDir` is present: `You are operating in an isolated git worktree at <path>.` This is injected for task execution and task-followup chat system prompts in API adapter paths.
...
- **Sequential local fast-forward merges**: `MergeBranch` now auto-updates stale task branches before `--ff-only` merges. If target branch has advanced (e.g., Task A already merged), service auto-rebases task worktree branch onto current target branch, then retries fast-forward merge so Task B can merge without manual CLI rebase. Real rebase conflicts are preserved (`merge_status=conflict`) with rebase abort + actionable error messaging.
...
- **Blocked child pre-creation for visibility**: when chain config is enabled on a task, a `StatusBlocked` child is immediately created and visible on the kanban board. When the parent completes, the blocked child is activated in place (prompt updated, status → pending, submitted to worker). Migration 069 adds `blocked` to the task status CHECK constraint
...
- **Changes tab scroll preservation**: SSE-triggered diff updates (`_updateDiffViewer`) use `fetch()` + offscreen DOM fingerprint comparison to skip DOM mutation entirely when the diff hasn't changed. Only when content actually changes does the function swap `innerHTML`, save/restore `window.scrollX`/`scrollY` and the active diff view mode (inline/split) via `requestAnimationFrame`, and let `window._diffFileState` session state restore file expand/collapse. The previous `htmx.ajax()` approach swapped the live DOM before the fingerprint check could fire, causing full DOM remounts and viewport jumps every 2 seconds during live task execution
...
- Projects now support `repo_url` in addition to `repo_path`. Create/Edit source modes are controlled only by `OPENVIBELY_ENABLE_LOCAL_REPO_PATH` for `Local Path` availability; unset/invalid defaults to GitHub-only mode. GitHub URL mode clones into managed storage (`PROJECT_REPO_ROOT`, default `./repos`, Docker default `/data/repos`) and Edit performs re-clone swap behavior. When local mode is disabled, legacy local-path projects still show a repo source selector with both "Local Path (existing)" and "GitHub URL" options, allowing users to switch to Git URL without being locked into the read-only local path. Git operations auto-detect system SSL CA bundles (Debian/Ubuntu/Alpine: `/etc/ssl/certs/ca-certificates.crt`, RHEL/CentOS: `/etc/pki/tls/certs/ca-bundle.crt`, etc.) and automatically fall back to `GIT_SSL_NO_VERIFY=true` if no valid bundle is found, preventing clone failures on misconfigured systems (logs a warning); users can override via `GIT_SSL_CAINFO` or explicitly set `GIT_SSL_NO_VERIFY` env var.
...
- Failed tasks now keep `Local` merge actions visible in the Changes-tab Actions dropdown even when `tasks.merge_status=merged`, so users can reconcile/merge additional uncommitted worktree edits without manual CLI intervention.
...
- Chat/thread display cleaning strips `<proposed_plan>` wrapper tags from rendered assistant content while preserving the inner plan text; raw stored output still retains wrappers so plan-complete CTA detection remains accurate
...
- Chat/thread markdown rendering now escapes raw HTML-like tags (outside fenced/inline code) before `marked.parse`, preventing malformed model outputs (e.g. pasted template fragments or `<function_calls>` blocks) from breaking thread DOM structure
...
- `/chat` plan-complete CTA state is now **server-derived**: `chatHistoryHasPlanCompletion()` in `chat_handler.go` checks whether the latest completed execution contains `<proposed_plan>` and passes `latestPlanComplete bool` to the template. The CTA div renders `data-has-plan-completion="true"|"false"` as a server-rendered attribute. The JS evaluator checks this attribute as a fallback on page load/refresh/HTMX navigation when ephemeral JS state is unavailable. `hidePlanCompletionPrompt()` clears the attribute to prevent re-showing after explicit dismiss.
...
- Chat/thread markdown links and task-result links now share a global link token (`--ov-link-color: #7480ff`) in `layout/base.templ` with explicit hover/focus/active/visited states, including links rendered inside tool-call IN/OUT content and `[TASK_EDITED]` conversions.
- `/schedule` current-time timeline tracer (dashed pre-day segment plus solid in-day line and endpoint dots) uses the same shared `--ov-link-color` token instead of hardcoded green so tracer color is consistent across light/dark themes.
- Schedule `Run At` controls now expose click-anywhere picker behavior in both the `New Scheduled Task` modal (`/schedule`) and task-detail schedule add/edit forms (`/tasks/:id` Schedules tab): clickable wrapper delegates to the native datetime picker (`showPicker()` with `focus()` fallback), and interactive input/wrapper surfaces use pointer-cursor affordance while preserving keyboard entry/focus behavior.
- Schedule create/edit repeat controls are now parity-aligned: all schedule forms (`/schedule` create modal + task-detail add/edit forms) include `Repeat Every`, hide + disable interval input for `once`, require/validate whole-number intervals (`>=1`) for repeating types, and submit `repeat_interval` consistently so values like every 2 days / every 3 hours persist and execute correctly.
- `/schedule` New Scheduled Task modal now defaults `Repeat` to `Daily` (initial option + modal reset behavior), uses a balanced responsive grid layout for `Run At`/`Repeat`/`Repeat Every`, and treats missing `repeat_type` in schedule-page task create submissions as `daily` server-side to keep UI default and persistence behavior aligned.
...
- `/channels` channel-card kebab menus now use consistent destructive actions across providers: `Delete` label (not `Remove`), `text-error` styling, and per-provider confirmation copy. Telegram card now includes a first-class delete action (`POST /channels/telegram/remove`) that clears Telegram channel settings.
...
- Task-card delete `X` button keeps cross-theme parity in resting state: light mode no longer applies a default circular background fill, matching dark-mode neutral rendering while preserving hover/error treatment.
- Sidebar footer user controls now use a user-menu dropdown in both expanded and collapsed sidebar states: clicking the avatar/name trigger opens a menu that contains `Logout`, replacing the prior always-visible inline `Logout` button. Footer layout keeps user trigger and theme toggle vertically centered with explicit focus-visible rings and minimum hit-target sizing classes (`sidebar-user-trigger`, `sidebar-user-menu-item`, `sidebar-user-collapsed-trigger`) for keyboard/a11y parity in light and dark themes.
...
- OpenAI streaming pseudo-tool sanitizer now flushes buffered text on UTF-8 rune boundaries (not raw byte offsets). This prevents multi-byte punctuation like `’` from being split across SSE deltas and rendering as replacement characters (`I���ve`) in live chat/task streams.
