...
- Anthropic URL-prompt recovery: if a turn ends (`end_turn`) after provider `code_execution`/`bash_code_execution` returns `too_many_requests` and no web tool ran, the agentic loop injects one steering retry turn to force `web_fetch`/`web_search` instead of finalizing with a “can’t fetch” memory-only summary.
...
- Anthropic URL prompts now prefer provider-native web retrieval by suppressing default local coding tools (`SkipDefaultTools=true`) whenever the prompt contains an explicit URL, so the model cannot detour into local `bash curl` retrieval after provider `code_execution` rate limits.
