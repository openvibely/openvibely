...
- Reasoning-effort changes between supported Astra Responses turns can be sent as a `configuration_update` input item so the request-level effort stays stable for cache reuse. This is Astra-only; non-Astra OpenAI models, OpenAI-compatible models, and unsupported transports do not receive `configuration_update` items. When Astra is selected as a mixture aggregator, its aggregator request remains a normal Astra Responses call and may use `configuration_update`; mixture reference models are unaffected.
- Provider-async function tools are supported only for first-party OpenAI `gpt-6-astra` Responses turns when OpenVibely has a durable execution record. OpenVibely may mark a narrow allowlist of approval-safe read runtime tools, such as selected memory/skill reads and bounded task/schedule/Pulse/usage discovery, with `async: true`. The async flag is never added for non-Astra models, OpenAI-compatible configs, Anthropic, Ollama, mixture aggregator or reference calls, completions fallback, default local file/shell tools, side-effecting runtime tools, or unallowlisted runtime tools.
