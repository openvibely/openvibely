...
- Mid-turn steering can be delivered as `response.steer` only while the active Astra WebSocket response is steerable. If the WebSocket is unavailable, the response already completed, the stream falls back to HTTP, or OpenAI rejects the steer event before accepting it, OpenVibely falls back to its normal queued follow-up or cancellation semantics and does not claim mid-turn delivery. After OpenAI accepts a steer, a disconnect is persisted as an unknown provider outcome: the instruction is neither marked applied nor automatically replayed until a successor response or explicit failure resolves it.
