...
OpenVibely currently budgets Anthropic requests with a 200k context window. Claude Code's separate `[1m]` model selections are not exposed; provider support for 1M context does not enable that mode in OpenVibely.
...
| Claude Mythos 5 (`claude-mythos-5`) | low / medium / high / xhigh / max | Limited-availability model. 128k max output; adaptive thinking is always on. |
| Claude Opus 4.8 (`claude-opus-4-8`) | low / medium / high / xhigh / max | 128k max output. |
| Claude Opus 4.7 (`claude-opus-4-7`) | low / medium / high / xhigh / max | 128k max output. |
...
`gpt-6-astra`, `gpt-6-sol`, and `gpt-6-luna` use the same OpenAI API key or OAuth configuration as other first-party OpenAI models. They do not change the default model, provider, credential type, or endpoint family.

GPT-6 workflow behavior is gated to those three exact first-party model IDs:

- Mid-turn steering can be delivered as `response.steer` only while the active GPT-6 WebSocket response is steerable. If the WebSocket is unavailable, the response already completed, the stream falls back to HTTP, or OpenAI rejects the steer event before accepting it, OpenVibely falls back to its normal queued follow-up or cancellation semantics and does not claim mid-turn delivery. A successor `response.created` commits an accepted steer and records it as applied atomically. If the connection ends first, OpenVibely preserves the instruction as an unknown provider outcome without replaying it; after the execution stops, the composer surfaces it as uncertain so the user can discard it or explicitly resend it.
...
- Reasoning-effort changes between supported GPT-6 Responses turns can be sent as a `configuration_update` input item so the request-level effort stays stable for cache reuse. Non-GPT-6 OpenAI models, OpenAI-compatible models, and unsupported transports do not receive `configuration_update` items. When a GPT-6 model is selected as a mixture aggregator, its aggregator request remains a normal Responses call and may use `configuration_update`; mixture reference models are unaffected.
- Provider-async function tools are supported only for first-party OpenAI GPT-6 Responses turns when OpenVibely has a durable execution record. OpenVibely may mark a narrow allowlist of approval-safe read runtime tools, such as selected memory/skill reads and bounded task/schedule/Pulse/usage discovery, with `async: true`. The async flag is never added for non-GPT-6 models, OpenAI-compatible configs, Anthropic, Ollama, mixture aggregator or reference calls, completions fallback, default local file/shell tools, side-effecting runtime tools, or unallowlisted runtime tools.
- Async tool calls are persisted by original `call_id` before local execution. Results are executed through the same runtime-tool authorization and ownership checks as synchronous calls, then returned in a later Responses request as `function_call_output` using that original `call_id`. Pending records survive restart; duplicate delivery attempts are idempotent, and provider rejection, stale responses, cancellation, and expiry move the durable record to terminal states instead of replaying or widening tool grants.
- Astra supports `low`, `medium`, `high`, `xhigh`, and `max` reasoning effort. `none` effort and temperature are unsupported for Astra; Sol and Luna retain their model-specific settings described above.
