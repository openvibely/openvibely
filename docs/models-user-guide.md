...
- Responses Lite does not support standard Responses async function tools. GPT-6 runtime tools are therefore sent without `async: true` and execute through the normal synchronous tool loop. The durable async-tool records remain reserved for a future non-Lite transport that explicitly supports acknowledgement and resume-sampling events.
