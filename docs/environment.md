...
| `OPENVIBELY_CODEX_REASONING_EFFORT` | Fallback reasoning effort for Codex requests when model config does not set one | `none`, `low`, `medium`, `high`, `xhigh`, `max` (model support varies; GPT-6 Astra does not support `none`) | If unset/invalid, uses the selected model's default: `medium` for GPT-6 Astra, GPT-5.6, GPT-5.5, GPT-5.4, and GPT-5.4 mini; `high` for most older Codex models | `medium` \| `none` |
