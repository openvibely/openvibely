...
| `CLAUDE_CODE_MAX_OUTPUT_TOKENS` | Overrides the per-call output token budget for Anthropic API/OAuth calls. When set to a positive integer, the value is applied up to the model's hard upper limit. | Positive integer string | Defaults to `64 000` for Claude Opus 5, Sonnet 5, Fable 5/5.1, Mythos 5, and Opus 4.6–4.8; `32 000` for Sonnet 4.6 and most Claude 4/3.7 models; `8 192` for Claude 3.5; and `4 096` for older Claude 3 Opus/Haiku models. Model-specific upper limits still apply. | unset \| `32000` |
