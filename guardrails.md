...
- In `/chat` plan mode, hide read-only exploration tool cards (`read_file`, `list_files`, `grep_search`) in rendered assistant bubbles for live/streaming turns; keep persisted history bubbles visible on refresh by skipping suppression when rendering `data-raw-content` history containers without `data-exec-id`
...
- Changes-tab live diff updates (`_updateDiffViewer`) must save and restore scroll position (`window.scrollX`/`scrollY`) and diff view mode (inline/split) around innerHTML swaps, and use a content fingerprint to skip DOM replacement when the diff hasn't changed. Without this, SSE diff_snapshot events every 2s cause viewport jumps while users are reviewing hunks.
...
- The tab visibility manager closes and recreates SSE on hide/show; the `onopen` handler that refreshes chat content must pass the current project and target `#chat-page-root` only
