...
- `/chat` mode-selector hydration must trigger `evaluatePlanCompletionPrompt()` after persisted mode restore (localStorage) and on selector change. Without this, focus-return HTMX refreshes can briefly evaluate in default `orchestrate` mode, hide `Switch to Orchestrate`, and never re-show until manual refresh.
